...
$barRef.addEventListener('click', () => {
  if (readerSutra && currentPanel === 'reader') {
    showPanel('list');
  } else if (currentPanel === 'list' && readerList.length) {
    showPanel('reader');
  }
});
...
function createDhatuCard(d) {
  ...
  const padaMap = { P: 'परस्मैपद', A: 'आत्मनेपद', U: 'उभयपद' };
  row.appendChild(devEl('span', 'sutra-badge badge-V', padaMap[d.pada] || d.pada));

  const detail = document.createElement('div');
  detail.className = 'sutra-detail';

  detail.appendChild(devEl('div', 'detail-sutra-full', d.aupadeshik || d.dhatu));

  const stripParts = [
    GANA_NAMES_DEV[+d.gana],
    PADA_DETAIL_DEV[d.pada],
    KARMA_DEV[d.karma],
    SETTVA_DEV[d.settva],
  ].filter(Boolean);
  if (stripParts.length) {
    const strip = document.createElement('div');
    strip.className = 'dhatu-strip';
    stripParts.forEach((part, i) => {
      if (i > 0) strip.appendChild(document.createTextNode(' · '));
      strip.appendChild(devEl('span', '', part));
    });
  ...
  if (d.artha) {
    ...
    lbl.textContent = 'अर्थः';
    sec.appendChild(lbl);
    sec.appendChild(devEl('div', 'detail-sanskrit', d.artha));
    detail.appendChild(sec);
  }
  if (d.artha_english) {
    const sec = document.createElement('div');
    ...
    en.className = 'detail-english';
    en.textContent = d.artha_english;
    sec.appendChild(en);
    detail.appendChild(sec);
  }
  if (d.dhaturoopnandini_note) {
    const sec = document.createElement('div');
    sec.className = 'detail-section';
    const lbl = document.createElement('div');
    lbl.className = 'detail-label';
    ...
    sec.appendChild(lbl);
    sec.appendChild(devEl('div', 'commentary-text', d.dhaturoopnandini_note));
    detail.appendChild(sec);
  }
  if (d.upasargas && d.upasargas.length) {
    const sec = document.createElement('div');
    sec.className = 'detail-section';
    const lbl = document.createElement('div');
    lbl.className = 'detail-label';
    lbl.textContent = 'उपसर्गाः';
    sec.appendChild(lbl);
    const grid = document.createElement('div');
    grid.className = 'upasarga-grid';
    for (const u of d.upasargas) {
      const item = document.createElement('div');
      item.className = 'upasarga-item';
      item.appendChild(devEl('span', 'upasarga-name', u.name));
      const meaning = document.createElement('span');
      meaning.className = 'upasarga-meaning';
      meaning.textContent = u.artha_hindi || '';
...
document.addEventListener('keydown', e => {
  ...
  if (e.key === 'ArrowRight' && readerSutra && readerIdx < readerList.length - 1)
    showReader(readerList[readerIdx + 1], readerIdx + 1);
  if (e.key === 'ArrowLeft' && readerSutra && readerIdx > 0)
    showReader(readerList[readerIdx - 1], readerIdx - 1);
});
